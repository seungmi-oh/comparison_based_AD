...
def main(c):
    ...
    if c.is_train==False:
        ...
        txt_file.close()
        if len(c.att_type)>0:
            send_email(f'[{c.data_settings.upper()}, {c.com_num}] {c.network_arch}-{c.att_type}-{c.infer_type} inference process is finish!', dict_string+'\n'+log_str) 
        else:
            send_email(f'[{c.data_settings.upper()}, {c.com_num}] {c.network_arch}-{c.infer_type} inference process is finish!', dict_string+'\n'+log_str) 
    else:
        log_str = ''
        if len(c.att_type)>0:
            send_email(f'[{c.data_settings.upper()}, {c.com_num}] {c.network_arch}-{c.att_type}-{c.train_type} training process is finish!', dict_string+'\n'+log_str) 
        else:
            send_email(f'[{c.data_settings.upper()}, {c.com_num}] {c.network_arch}-{c.train_type} training process is finish!', dict_string+'\n'+log_str) 
