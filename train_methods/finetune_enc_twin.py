...
def train_meta_epoch(c, model_cfg, epoch, loader, model, optimizers, pool_layers_names, fe_loss_fn, txt_file):
    ...
    if epoch ==0:
        encoder.freeze_all_layers()
        if len(c.att_type)>0:
            encoder.unfreeze_att_conv_layers()
        else:
            pass
    elif epoch ==c.freeze_enc_epochs:
        encoder.unfreeze_all_layers()
    else:
        pass
    ...
    for sub_epoch in range(c.sub_epochs):
        ...
        for i in range(I):
            ...
            image, label, mask = next(dataloader_inf)
...
def test_meta_epoch(c, model_cfg, epoch, loader, model, pool_layers_names, fe_loss_fn, txt_file):
    ...
    with torch.no_grad():
        for i, (image, label, gt_mask) in enumerate(tqdm(loader, disable=c.hide_tqdm_bar)):
            ...
            # save feature map
            save_features = (c.viz_diff_features or c.viz_total_features)
            if c.is_train ==False and save_features ==True:
                assert c.viz_total_features != c.viz_diff_features
                f_num=0
                for pl, l in enumerate(c.nf_layers):
                    ...
                    if c.viz_total_features ==True:
                        ...
                        rng = range(2,len(encoder.feats[l_idx]),3)
...
# just infernce models without calculating loss 
def inference_models(c, model_cfg, epoch, loader, model, pool_layers_names, fe_loss_fn, txt_file):
    # test
    if c.verbose:
        print('\nInference models on test set:')
    txt_file.write('\nInference models and scores on test set:')
    #
    encoder = model[0]
    decoder = model[1]
    encoder = encoder.eval()
...
def run(c, data_cfg, model_cfg):
    if c.is_train ==True:
        ...
        if c.viz_total_features==True:
            dir_feature.append('viz_total_feat')
        if c.viz_diff_features==True:
            dir_feature.append('viz_diff_feat')
    ...
    if 'resnet' in c.network_arch:
        optimizers = torch.optim.AdamW([{'params': encoder.parameters()}, {'params': decoder.parameters()}], lr=c.lr, weight_decay=c.w_decay)
    elif 'visformer' in c.network_arch:
        optimizers = torch.optim.AdamW([{'params': encoder.parameters()}, {'params': decoder.parameters()}], lr=c.lr, weight_decay=c.w_decay)
    else:
        NotImplementedError('check optimizers')
    ...
    if c.aug_ratio_train>0:
        ...
        train_dataset = Dataset(c.img_size, c.train_aug_data_path, data_cfg.class_name.train, c.norm_mean, c.norm_std, is_pairset = c.is_pairset, is_train=True)
        if c.repeat_num >1:
    ...
    for epoch in range(c.meta_epochs+c.freeze_enc_epochs):
        if c.is_train==True:
            ...
            if c.test_data_type == 'aug':
                ...
                if c.th_manual>0:
                    test_aug_image_list, gt_aug_label_list, gt_aug_mask_list, pred_aug_list, feature_map_aug_list= inference_models(c, model_cfg, epoch, test_aug_loader, model, c.pool_layers_dec, fe_loss_fn, txt_file)
                else:
                    test_aug_image_list, gt_aug_label_list, gt_aug_mask_list, pred_aug_list, feature_map_aug_list= test_meta_epoch(c, model_cfg, epoch, test_aug_loader, model, c.pool_layers_dec, fe_loss_fn, txt_file)
            else:
                if c.th_manual>0 or c.fn >=0:
                ...
                if c.th_manual>0:
                    test_image_list, gt_label_list, gt_mask_list, pred_list, feature_map_list= inference_models(c, model_cfg, epoch, test_loader, model, c.pool_layers_dec, fe_loss_fn, txt_file)
                else:
                    test_image_list, gt_label_list, gt_mask_list, pred_list, feature_map_list= test_meta_epoch(c, model_cfg, epoch, test_loader, model, c.pool_layers_dec, fe_loss_fn, txt_file)
        ...
        if c.is_train==True:
            if epoch % c.eval_epoch ==c.eval_epoch-1 or epoch == c.meta_epochs+c.freeze_enc_epochs-1:
                anomaly_cal = Anomaly_Score_Calculator(c.nf_layers, c.crp_size, [], [], [], [], [], c.train_type, c.pro) 
                ...
                anomaly_cal.save_results(c, epoch, [], [], pred_list, gt_mask_list, gt_label_list, [], test_dataset, data_cfg.full_dims.train, det_roc_obs, seg_roc_obs, seg_pr_obs, '', txt_file, result_dict, c.is_full)
            save_weights(c, model, c.model_dir)  
        else:
            anomaly_cal = Anomaly_Score_Calculator(c.nf_layers, c.crp_size, [], [], [],[], [], c.infer_type, c.pro) 
            if c.test_data_type == 'aug':
                if c.th_manual>0 or c.fn >=0:
