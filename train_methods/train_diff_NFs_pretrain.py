...
def test_meta_epoch(c, epoch, loader, model, pool_layers_names, txt_file):
    ...
    height = list()
    width = list()
    image_list = [list() for l in range(2)]
    gt_label_list = list()
    gt_mask_list = list()
    pred_list = list()
    ...
    with torch.no_grad():
        for i, (image, label, gt_mask) in enumerate(tqdm(loader, disable=c.hide_tqdm_bar)):
            ...
            for l_idx, l in enumerate(c.nf_layers):
                ...
                total_features = (c.save_features or c.viz_total_features)
                if c.is_train ==False and save_features ==True:
                    assert total_features != c.viz_diff_features
                    if total_features ==True:
        ...
        if c.is_train ==True:
            ...
            if epoch % c.eval_epoch ==c.eval_epoch-1 or epoch == c.meta_epochs+c.freeze_enc_epochs-1:
                ...
                if c.nf_aug_ratio_train>0 or c.repeat_num>1:
                    _, _, _, train_dist, _, train_gt_mask_list, _, train_pred_list= test_meta_epoch(c, epoch, train_val_loader, model, pool_layers_names, txt_file)
                else:
                    _, _, _, train_dist, _, train_gt_mask_list, _, train_pred_list= test_meta_epoch(c, epoch, train_loader, model, pool_layers_names, txt_file)
        else:
            ...
            if c.test_data_type == 'aug':
                ...
                _, _, _, train_dist, _, train_gt_mask_list, train_feature_map_list, _= test_meta_epoch(c, epoch, train_loader, model, pool_layers_names, txt_file)
