...
def train_meta_epoch(c, epoch, loader, model, optimizers, pool_layers_names, txt_file):
    ...
    for sub_epoch in range(c.sub_epochs):
        ...
        for i in range(I):
            ...
            # encoder prediction
            image1, image2 = image
            image1 = image1.to(c.device)  # single scale
            image2 = image2.to(c.device)  # single scale

            if c.nf_aug_ratio_train >0:
                mask1, mask2 = mask
                mask1 = mask1.to(c.device)  
                mask2 = mask2.to(c.device)  
            ...
            for l_idx, l in enumerate(c.nf_layers):
                ...
                # get loss 
                nf_log_prob, defect_logp, good_logp = get_logp_2d(C, z, log_jac_det, m, True)
...
def test_meta_epoch(c, model_cfg, epoch, loader, model, pool_layers_names, fe_loss_fn, txt_file):
    ...
    height = list()
    width = list()
    image_list = [list() for i in range(2)]
    gt_label_list = list()
    gt_mask_list = list()
    ...
    with torch.no_grad():
        for i, (image, label, gt_mask) in enumerate(tqdm(loader, disable=c.hide_tqdm_bar)):
            # save
            image1, image2 = image
            image_list[0].extend(t2np(image1))
            image_list[1].extend(t2np(image2))
            ...
            for pl, l in enumerate(c.nf_layers):
                ...
                # save feature maps
                save_features = (c.save_features or c.viz_diff_features or c.viz_total_features)
                diff_features = (c.save_features or c.viz_diff_features)
                if c.is_train ==False and save_features ==True:
                    assert diff_features != c.viz_total_features
                    if diff_features ==False:
                        rng = range(len(encoder.feats[l_idx]))
                    else:
                        rng = list(range(2,len(encoder.feats[l_idx]),3)) + [len(encoder.feats[l_idx])-1] 

                    for f_idx in rng:
                        f = encoder.feats[l_idx][f_idx]
                        _, c_idx = torch.topk(torch.mean(f, (-2,-1)), int(c.feat_avg_topk*f.size(1)), 1)

                        feat_map_sorted = [] 
                        for b in range(c_idx.size(0)):
                            feat_map_sorted.append(f[b,c_idx[b],:,:])
                        feat_map_sorted_mean = torch.unsqueeze(torch.mean(torch.stack(feat_map_sorted,0), 1), 1)
                        feat_map = F.interpolate(feat_map_sorted_mean, size=(image1.size(-2), image1.size(-1)), mode = 'bicubic', align_corners =True)
                        if i==0:
                            feature_maps.append([])
                ...
                # get loss
                nf_log_prob, defect_logp, good_logp= get_logp_2d(C, z, log_jac_det, m, False)
...
def test_meta_fps(c, model_cfg, epoch, loader, model, pool_layers_names, fe_loss_fn, txt_file, result_dict):
    ...
    with torch.no_grad():        
        ...
        for i, (image, _, _) in enumerate(tqdm(loader, disable=c.hide_tqdm_bar)):
            image1, image2 = image
            image1 = image1.to(c.device)  
            image2 = image2.to(c.device)  
            enc_out = encoder(image1, image2)  
            if c.add_fe_anomaly==True:
                if model_cfg.backbone.skip_connection ==True:
                    pix_out = decoder(enc_out)
...
def run(c, data_cfg, model_cfg):
    ...
    load_weights(c, model_loaded, 'twin_fe_only')
    ...
    if c.is_train:
        ...
        for i, nl in enumerate(c.nf_layers):
            ...
            e = activation[layer].detach()  # BxCxHxW
            # indiv_pr_obs['exp-layerwise'].append(Score_Observer(f'PIX_AUPR (exp-layerwise, {layer})'))
            # indiv_pr_obs['exp_std-layerwise'].append(Score_Observer(f'PIX_AUPR (exp_std-layerwise, {layer})'))
            # indiv_pr_obs['gauss_cdf-layerwise'].append(Score_Observer(f'PIX_AUPR (gauss_cdf-layerwise, {layer})'))
            input_dims = [e.size(-3), e.size(-2), e.size(-1)]
    ...
    for epoch in range(c.meta_epochs):
        if c.is_train ==True:
            ...
            if c.test_data_type == 'aug':
                ...
                height, width, test_aug_image_list, test_aug_dist, gt_aug_label_list, gt_aug_mask_list, feature_map_list, pred_aug_list= test_meta_epoch(c, model_cfg, epoch, test_aug_loader, model, pool_layers_names, fe_loss_fn, txt_file)
                ...
                if c.save_features ==True:
                    feat_dirs = os.path.join(c.model_dir, data_cfg.class_name.test, c.infer_type, 'feat_hist')
                    makedirs(feat_dirs)
                    npz_fname = os.path.join(feat_dirs, 'feat_maps.npz')
                    np.savez(npz_fname, train_feat_maps = train_feature_map_list, test_feat_maps=feature_map_list, gt = gt_mask_list)
                    break
                else:
                    pass
        ...
        if c.is_train ==True:
            if epoch % c.eval_epoch ==c.eval_epoch-1 or epoch == c.meta_epochs+c.freeze_enc_epochs-1:
                ...
                anomaly_cal.save_results(c, epoch, [], test_dist, [], gt_mask_list, gt_label_list, [], test_dataset, data_cfg.full_dims.train, det_roc_obs, seg_roc_obs, seg_pr_obs['exp_std-layerwise'], '', txt_file, result_dict, c.is_full, indiv_pr_obs=indiv_pr_obs['exp_std-layerwise'], prob_mapping='exp_std', inverting_mode='layerwise')
            ...
            if c.test_data_type == 'aug':
                if c.th_manual>0 or c.fn >=0 or c.save_features ==True:
                    anomaly_cal.save_results(c, epoch, test_aug_image_list, test_aug_dist, pred_aug_list, gt_aug_mask_list, gt_aug_label_list, feature_map_aug_list, test_aug_dataset, data_cfg.full_dims.train, aug_det_roc_obs, aug_seg_roc_obs, aug_seg_pr_obs, save_dir, txt_file, result_dict, c.is_full, False, indiv_pr_obs=[], prob_mapping='exp_std', inverting_mode='layerwise')
                else:
                    anomaly_cal.save_results(c, epoch, test_aug_image_list, test_aug_dist, pred_aug_list, gt_aug_mask_list, gt_aug_label_list, feature_map_aug_list, test_aug_dataset, data_cfg.full_dims.train, aug_det_roc_obs, aug_seg_roc_obs, aug_seg_pr_obs, save_dir, txt_file, result_dict, c.is_full, True, indiv_pr_obs=[], prob_mapping='exp_std', inverting_mode='layerwise')
                if c.fn>=0:
                ...
                if c.th_manual>0 or c.fn >=0 or c.save_features ==True:
                    anomaly_cal.save_results(c, epoch, test_image_list, test_dist, pred_list, gt_mask_list, gt_label_list, feature_map_list, test_dataset, data_cfg.full_dims.test, det_roc_obs, seg_roc_obs, seg_pr_obs, save_dir, txt_file, result_dict, c.is_full, False, indiv_pr_obs=indiv_pr_obs, prob_mapping='exp_std', inverting_mode='layerwise')
                else:
                    anomaly_cal.save_results(c, epoch, test_image_list, test_dist, pred_list, gt_mask_list, gt_label_list, feature_map_list, test_dataset, data_cfg.full_dims.test, det_roc_obs, seg_roc_obs, seg_pr_obs, save_dir, txt_file, result_dict, c.is_full, True, indiv_pr_obs=indiv_pr_obs, prob_mapping='exp_std', inverting_mode='layerwise')
                if c.fn>=0:
