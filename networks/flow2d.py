'''This code is based on the CS-Flow project (source: https://github.com/marco-rudolph/cs-flow).
...
class Convolutions(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, exp_ch = 2.0,
        ...
        self.conv_scale_0 = nn.Conv2d(in_channels, int(in_channels * exp_ch), kernel_size=kernel_size, padding=pad, bias=not batch_norm, padding_mode=pad_mode)
        self.conv_scale_1 = nn.Conv2d(int(in_channels * exp_ch) , out_channels, kernel_size=kernel_size, padding=pad, bias=not batch_norm, padding_mode=pad_mode, dilation=1)

        self.lr = nn.LeakyReLU(self.leaky_slope)

    def forward(self, x):
        y = self.lr(self.conv_scale_0(x))
        out = self.conv_scale_1(y)
...
class ParallelPermute(nn.Module):
    ...
    def __init__(self, dims_in, seed):
        ...
        np.random.seed(seed)
        perm, perm_inv = self.get_random_perm()
        self.perm = [perm]
        self.perm_inv = [perm_inv]

    def get_random_perm(self):
        perm = np.random.permutation(self.in_channels)
    ...
    def forward(self, x, rev=False):
        if not rev:
            shuffle_x_list = []
            shuffle_x = x[0][:, self.perm[0]]
            shuffle_x_list.append(shuffle_x)
            return shuffle_x_list
        else:
            inv_shuffle_x_list = []
            inv_shuffle_x = x[0][:, self.perm_inv[0]]
            inv_shuffle_x_list.append(shuffle_x)
            return inv_shuffle_x_list
...
class parallel_flowpp_coupling_layer(nn.Module):
    '''Convolutional blocks for shift and scale parameters'''
    def __init__(self, dims_in, F_class=Convolutions, F_args={}, clamp=5., k=3):
        super(parallel_flowpp_coupling_layer, self).__init__()
        channels = dims_in[0]
        self.ndims = len(dims_in)

        self.split_len1 = channels // 2 
        self.split_len2 = channels - channels // 2
        self.k = k # the number of logistic cdf functions
        ...
        self.k = k
        self.s1 = F_class(self.split_len1, self.split_len2 * (2 + 3 * self.k), **F_args)
        self.s2 = F_class(self.split_len2, self.split_len1 * (2 + 3 * self.k), **F_args)
    ...
    def forward(self, x, rev=False):
        # split inputs following channel dimension
        x1 = x[0].narrow(1,0,self.split_len1)
        x2 = x[0].narrow(1, self.split_len1, self.split_len2)
        if not rev:
            r2 = self.s2(x2)

            r2 = r2.view([x1.size(0), 2+self.k*3, self.split_len1]+ list(x1.size()[2:]))
            a2, b2, pi2, mu2, s2 = torch.split(r2, (1,1,self.k, self.k, self.k), dim=1) 
            a2 = a2.squeeze(1)
            b2 = b2.squeeze(1)
             
            out1 = mixture_log_cdf(x1, pi2, mu2, s2).exp()
            out1, scale_ldj1 = inverse(out1)
            y1 = out1 * self.e(a2) + b2 
            logistic_ldj1 = mixture_log_pdf(x1, pi2, mu2, s2)
            sldj1 = self.log_e(a2)+ logistic_ldj1 + scale_ldj1

            r1 = self.s1(y1)

            r1 = r1.view([x2.size(0), 2+self.k*3, self.split_len2]+ list(x2.size()[2:]))
            a1, b1, pi1, mu1, s1 = torch.split(r1, (1,1,self.k, self.k, self.k), dim=1) 
            a1 = a1.squeeze(1)
            b1 = b1.squeeze(1)
             
            out2 = mixture_log_cdf(x2, pi1, mu1, s1).exp()
            out2, scale_ldj2 = inverse(out2)
            y2 = out2 * self.e(a1) + b1 
            logistic_ldj2 = mixture_log_pdf(x2, pi1, mu1, s1)
            sldj2 = self.log_e(a1)+ logistic_ldj2 + scale_ldj2
            ...
            out2 = (x2-b1)/self.e(a1)
            out2, scale_ldj2 = inverse(out2)
            y2 = mixture_inv_cdf(out2, pi1, mu1, s1)
            logistic_ldj2 = mixture_log_pdf(out2, pi1, mu1, s1)
            sldj2 = self.log_e(a1)+ logistic_ldj2 + scale_ldj2
            ...
            out1 = (x1-b2)/self.e(a2)
            out1, scale_ldj1 = inverse(out1)
            y1 = mixture_inv_cdf(out1, pi2, mu2, s2)
            logistic_ldj1 = mixture_log_pdf(out1, pi2, mu2, s2)
            sldj1 = self.log_e(a2)+ logistic_ldj1 + scale_ldj1

        y = [torch.clamp(torch.cat((y1, y2),1), -1e6, 1e6)]
        self.last_jac = [torch.sum(sldj1, dim=1) +  torch.sum(sldj2, dim=1)]
        return y
...
class parallel_glow_coupling_layer(nn.Module):
    '''Convolutional blocks for shift and scale parameters'''
    def __init__(self, dims_in, F_class=Convolutions, F_args={},
                 clamp=5.):
    ...
    def forward(self, x, rev=False):
        ...
        if not rev:
            ...
            s2 = r2[:,:self.split_len1]
            t2 = r2[:,self.split_len1:]
             
            y1 = self.e(s2)*x1+t2 

            r1 = self.s1(y1)
            s1 = r1[:,:self.split_len2]
            t1 = r1[:, self.split_len2:]
        ...
        y = [torch.clamp(torch.cat((y1, y2),1), -1e6, 1e6)]
        self.last_jac = [torch.sum(self.log_e(s1), dim=1) +  torch.sum(self.log_e(s2), dim=1)]
        return y
...
class ReversibleGraphNet(nn.Module):
    ...
    def forward(self, x, rev=False):
        ...
        jacobian = self.jacobian(run_forward=False)
        if len(out) == 1:
            return out[0], jacobian[0]
        else:
            return out, jacobian 
