...
RE_LIST="${RE_LIST:-"0"}" # 반복 실행 인덱스 리스트 (override 가능)
...
META_EPOCHS_FE="${META_EPOCHS_FE:-40}"
FREEZE_ENC_EPOCHS="${FREEZE_ENC_EPOCHS:-0}"
...
META_EPOCHS_NF="${META_EPOCHS_NF:-24}"
...
run_fe() {
  ...
  python3 main.py \
    ...
    --train_type twin_fe_only --pretrained no --finetuning yes \
...
run_nf() {
  ...
  python3 main.py \
    $(common_base "$ds") \
    --att_type "$att" \
    --train_type twin_nf_only --pretrained no --finetuning yes \
    ...
    -nf_inp "$NF_INP" --eval_epoch "$EVAL_EPOCH_NF" --lr_cosine yes
}
