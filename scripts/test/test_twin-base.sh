...
VIZ_TOTAL_FEAT="${VIZ_TOTAL_FEAT:-no}" # yes/no
VIZ_DIFF_FEAT="${VIZ_DIFF_FEAT:-no}" # yes/no
...
common_base() {
  ...
  echo -run "$RUN" -nl "$NL" \
       ...
       --is_full "$IS_FULL" --k_size "$K_SIZE" --save_all_tn_sample "$SAVE_ALL_TN" --viz "$IS_VIS" --th_pix "$TH_PIX" --pro "$PRO" --is_open "$IS_OPEN" --is_close "$IS_CLOSE" --fn "$FN" --save_feat "$SAVE_FEAT" --viz_total_feat "$VIZ_TOTAL_FEAT" --viz_diff_feat "$VIZ_DIFF_FEAT" --th_manual "$TH_MANUAL"
}
